...
where
    T: for<'a> Message<'a, Error: Into<error::Error>> + Send,
{
    async fn write_in(
        &mut self,
        io: &mut (dyn AsyncWrite + Send + Unpin),
    ) -> Result<(), error::Error> {
        self.write_in(io).await.map_err(Into::into)
    }
}
...
//     async fn write_in(&mut self, io: impl AsyncWrite + Send + Unpin) -> Result<(), Self::Error> {
...
impl<'a, Fut, E> Future for BoxMessageFut<Fut>
where
